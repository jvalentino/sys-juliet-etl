...
You can right-click on the main application and run as a Spring Boot App, however I am also running it on port 8084 so I can run the rest app at the same time:

![01](wiki/run-as.png)

![01](wiki/server-port.png)

This will then be accessible via http://localhost:8084, which gives a status page:
...
If it worked you can access it via http://localhost:8084/swagger-ui/index.html
...
FROM openjdk:11
WORKDIR .
COPY build/libs/sys-juliet-etl-0.0.1.jar /usr/local/sys-juliet-etl-0.0.1.jar
EXPOSE 8080
